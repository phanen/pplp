...
int32_t
test_pplp_circuit (e_role role, const std::string &address, uint16_t port,
                   ...
                   e_mt_gen_alg mt_alg, e_sharing sharing, uint32_t thr)
{
  ...
  s_out = BuildPPLPCircuit (s_x0, s_x1, s_y0, s_y1, s_thr, circA, circY);
  ...
  std::cout << "Testing PPLP in " << get_sharing_name (sharing)
            << " sharing: " << std::endl
            << std::endl;
...
share *
BuildPPLPCircuit (share *x0, share *x1, share *y0, share *y1, share *thr,
