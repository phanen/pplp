...
int main(int argc, char *argv[]) {
  ...
  pplp_printf("Test start...\n");

  // test leg
  radius = 16;
  for (size_t i = 0; i < 9; ++i) {
    auto raw_data = test_server_leg(sockfd_client, radius, ip, port, xb, yb);
    parse2csv_server_leg("./server_legacy.csv", radius, i == 0, raw_data);
    radius <<= 1;
  }
  ...
  //   auto cur_vec = test_server_opt(sockfd_client, radius, ip, port, xb, yb);
  // }
...
test_server_leg(int sockfd_client, uint64_t radius, //
  ...
  pplp_printf("Receive parms from the client, bytes: %zu\n", size_t(bytes));
  ...
  pplp_printf("Receive the public key from the client, bytes: %zu\n",
              size_t(bytes));
  PublicKey pk;
  ...
  *(uint64_t *)bf_buf = w;
  ...
  pplp_printf(
      "Send the encrypted blind distance to the client, bytes sent: %zu\n",
      bytes);
