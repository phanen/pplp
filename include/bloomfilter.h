...
public:
    ...
    bloom_filter(const uint8_t *buf)
    {
        ...
        this->desired_false_positive_probability_ = p_head->desired_false_positive_probability_;
    ...
    {
        // std::cout << this->bit_table_.size() << ' ' << this->table_size_ << std::endl;
        // std::cout << this->salt_.size() << ' ' << this->salt_count_ << std::endl;
        size_t head_len = sizeof(bf_hdr); // 5*8+4 ?
        // std::cout << " sizeof header" << head_len << std::endl;
        size_t data_len = this->salt_.size() * sizeof(unsigned int) + this->bit_table_.size() * sizeof(unsigned char);
    ...
    void serialize(uint8_t *buf) // size of buf: compute_serialization_size()
    {
        ...
        p_head->desired_false_positive_probability_ = this->desired_false_positive_probability_;

        unsigned int *salt_begin = (unsigned int *)(p_head + 1);
        memcpy(salt_begin, this->salt_.data(), sizeof(unsigned int) * this->salt_.size());

        unsigned char *bt_begin = (unsigned char *)(salt_begin + this->salt_.size());
        memcpy(bt_begin, this->bit_table_.data(), sizeof(unsigned char) * this->bit_table_.size());
    }
